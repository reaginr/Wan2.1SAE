...
# 涉及的相关核心术语
# DiT diffusion transformer 是 Wan 模型的核心生成模块（负责从文本 / 图像生成视频帧，SAE提取中间参数进行解析应该是放在这一部分

# 这部分是使用的默认提示词，如果没有给出提示词的话会默认采用
EXAMPLE_PROMPT = {
    "t2v-1.3B": {
        "prompt":
...
def _validate_args(args):
    ...
    # 如果没有给出采样步数，对于i2v模型，采样步数会设置为40，t2v模型则是50
    if args.sample_steps is None:
        args.sample_steps = 50
        if "i2v" in args.task:
            args.sample_steps = 40
    # 如果没有给出采样偏移量，对于i2v模型，采样偏移量会设置为5，t2v模型则是3
    # sample_shift是Wan
    # 模型（基于流匹配调度器）特有的参数，核心作用是：控制视频生成时「帧与帧之间的运动流畅度 + 生成内容的稳定性」，可以理解为：
    # 本质：调整流匹配（Flow Matching）过程中，采样步骤的 “时间偏移”，影响视频帧的运动插值效果；
    # 通俗解释：
    # ✅ 数值越小 → 视频帧运动越 “紧凑”，但可能出现帧抖动、画面跳变；
    # ✅ 数值越大 → 视频帧过渡越平滑、运动轨迹越自然，但数值过大（比如超过16）可能导致画面模糊、细节丢失；
    # 适用场景：快速运动场景（比如示例的 “猫咪拳击”）：设8~10更合适，能体现激烈动作的流畅性；
    # 静态 / 慢动作场景：设3~5即可，避免画面过度模糊。
    if args.sample_shift is None:
        args.sample_shift = 5.0
        if "i2v" in args.task and args.size in ["832*480", "480*832"]:# i2v（图生视频）+ 832*480：图生视频本身有参考帧，偏移量设 3 能保证画面和原图一致，避免过度流畅导致偏离原图；
            args.sample_shift = 3.0
        elif "flf2v" in args.task or "vace" in args.task:# flf2v/vace：首尾帧生视频 / 风格迁移需要强帧间连贯性，偏移量设 16 能让首尾帧过渡更自然
            args.sample_shift = 16

    # 93-100 帧数，不过多解释，T2I是一帧，其他生成视频都是默认81帧
    # The default number of frames are 1 for text-to-image tasks and 81 for other tasks.
    if args.frame_num is None:
        args.frame_num = 1 if "t2i" in args.task else 81

    # 校验T2I模型的帧数设置 T2I frame_num check
    if "t2i" in args.task:
        assert args.frame_num == 1, f"Unsupport frame_num {args.frame_num} for task {args.task}"

    args.base_seed = args.base_seed if args.base_seed >= 0 else random.randint(
        0, sys.maxsize)
    # Size check，详见 config _init_.py文件中的SUPPORTED_SIZES字段
...
def _parse_args():
    ...
    # 现在模型的存放位置是,参数选择这个即可 E:/Wan2.1-T2V-1.3B
    parser.add_argument(
        "--ckpt_dir",
        type=str,
        default=None,
    ...
    #通俗解释：
    # offload = 卸载 / 转移。模型运行时会占用大量 GPU 显存（比如 1.3B 模型全加载要几 GB 显存），开启offload_model=True后，
    # 每次模型完成一次计算（前向传播），就把模型从 GPU 移回 CPU，只保留计算结果在 GPU。
    # 作用：
    # 大幅降低 GPU 显存占用（比如 8G 显存的显卡，不开可能溢出，开了就能运行），代价是生成速度略慢（因为要频繁在 CPU/GPU 间搬数据）。
    parser.add_argument(
        "--offload_model",
        type=str2bool,
        default=None,
